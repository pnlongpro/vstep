...
interface UploaderExamUploadModalProps {
  onClose: () => void;
}

export function UploaderExamUploadModal({ onClose }: UploaderExamUploadModalProps) {
  const [selectedSkill, setSelectedSkill] = useState<'reading' | 'listening' | 'writing' | 'speaking' | null>(null);

  const handleSave = (examData: any) => {
    console.log('Uploader tải đề thi:', examData);
    alert('Đã tải đề thi lên! Đang chờ Admin duyệt.');
    onClose();
  };
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl z-50 w-full max-w-4xl">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-6 text-white rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold">Tải đề thi mới</h3>
              <p className="text-sm opacity-90 mt-1">Chọn kỹ năng bạn muốn tải đề thi</p>
            </div>
        ...
        <div className="p-8">
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
            <p className="text-sm text-blue-800">
              ...
              Đề thi của bạn sẽ được Admin kiểm duyệt trước khi xuất hiện trên web.
            </p>
          </div>
        </div>
      </div>
