...
import { Search, Download, Book, Video, Layers } from 'lucide-react';
import { textbookMaterials, mediaMaterials, getClassMaterials, type ClassCategory } from './classMaterialsData';
...
export function ClassMaterialsTab() {
  ...
  // Get filtered class materials
  const filteredClassMaterials = getClassMaterials(
    classCategory,
    filterCourse,
    filterSkill,
    searchTerm
  ...
  const allMaterials = classCategory === 'textbook' ? textbookMaterials : mediaMaterials;
  const countByCourse = {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-1">{textbookMaterials.length + mediaMaterials.length}</h3>
      ...
      <div className="flex items-center justify-between">
        <p className="text-gray-600">
          Tìm thấy <span className="font-semibold text-emerald-600">{filteredClassMaterials.length}</span> tài liệu
        </p>
      </div>
